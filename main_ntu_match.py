...
seen_classes = list(set(range(120))-set(unseen_classes))  # ntu60
train_label_dict = {}
...
for idx, l in enumerate(range(120)):
    tmp = [0] * 120
    tmp[idx] = 1
...
class Processor():
    ...
    def __init__(self, arg):
        ...
        self.spatial_semantics = torch.load('/root/autodl-tmp/FGE/semantics/ntu120_spatial_semantic_feature.tar')
        self.temporal_semantics = torch.load('/root/autodl-tmp/FGE/semantics/ntu120_temporal_semantic_feature.tar')
        
        self.spatial_round_fg = torch.load('/root/autodl-tmp/FGE/semantics/hierarchical_phase_descriptions_onedialogue/ntu120_gpt4omini_incontext_spatial_fg.tar')
        self.spatial_round_mg = torch.load('/root/autodl-tmp/FGE/semantics/hierarchical_phase_descriptions_onedialogue/ntu120_gpt4omini_incontext_spatial_mg.tar')
        self.spatial_round_cg = torch.load('/root/autodl-tmp/FGE/semantics/hierarchical_phase_descriptions_onedialogue/ntu120_gpt4omini_incontext_spatial_cg.tar')
        self.temporal_round_cg = torch.load('/root/autodl-tmp/FGE/semantics/phase_descriptions_onedialogue/ntu120_gpt4omini_incontext_temporal_fp.tar')
        self.temporal_round_mg = torch.load('/root/autodl-tmp/FGE/semantics/phase_descriptions_onedialogue/ntu120_gpt4omini_incontext_temporal_fsp.tar')
        self.temporal_round_fg = torch.load('/root/autodl-tmp/FGE/semantics/phase_descriptions_onedialogue/ntu120_gpt4omini_incontext_temporal_fstp.tar')
    ...
    def train(self, epoch, save_model=False):
        ...
        spatial_semantics_seen = self.spatial_semantics[seen_classes].cuda(self.output_device)
        # temporal_semantics_seen = self.temporal_semantics[seen_classes].cuda(self.output_device)
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            output_spa, output_tem, _, _, _, _ = self.model(data, spatial_round_seen, temporal_round_seen, label_semantics_seen)
            # if epoch <=3:
            #     coffe_cg = 0.125 + 0.5 * (0.5 - 0.125) * (1 + np.cos(np.pi * epoch / 3))
            #     coffe_mg = 0.25 + 0.5 * (1 - 0.25) * (1 + np.cos(np.pi * epoch / 6))
            # elif (epoch>3 and epoch<=6):
            #     coffe_cg = 0.001
            #     coffe_mg = 0.001 + 0.5 * (1 - 0.001) * (1 + np.cos(np.pi * epoch / 6))
            # else:
            #     coffe_cg = 0.001
            #     coffe_mg = 0.001 
            ...
            # loss_lal = self.loss_ce(output_label, label_reindex_seen)
            
            # if epoch<=1:
            #     loss_spa = 0.5*self.loss_ce(output_spa[0], label_reindex_seen)
            #     loss_tem = 0.5*self.loss_ce(output_tem[0], label_reindex_seen) + 0.5*self.loss_ce(output_tem[1], label_reindex_seen)+0.5*self.loss_ce(output_tem[2], label_reindex_seen)
            # elif (epoch>=2 and epoch <4):
            #     loss_spa = 0.25*self.loss_ce(output_spa[0], label_reindex_seen) + 0.5*self.loss_ce(output_spa[1], label_reindex_seen)
            #     loss_tem = 0.5*self.loss_ce(output_tem[0], label_reindex_seen) + 0.5*self.loss_ce(output_tem[1], label_reindex_seen)+0.5*self.loss_ce(output_tem[2], label_reindex_seen)
            # else:
            #     loss_spa = 0.125*self.loss_ce(output_spa[0], label_reindex_seen) + 0.25*self.loss_ce(output_spa[1], label_reindex_seen)+0.5*self.loss_ce(output_spa[2], label_reindex_seen)
            #     loss_tem = 0.5*self.loss_ce(output_tem[0], label_reindex_seen) + 0.5*self.loss_ce(output_tem[1], label_reindex_seen)+0.5*self.loss_ce(output_tem[2], label_reindex_seen)
            
            # loss_ce = self.loss_ce(output[2], label_reindex_seen)
            # loss_cl = (self.loss_ce(cl_output[0], label_reindex_seen)+self.loss_ce(cl_output[1], label_reindex_seen)+self.loss_ce(cl_output[2], label_reindex_seen))/3
            # rl_label = torch.eye(55).cuda(self.output_device)
            # loss_rl = (self.loss_ce(rl_output[0], rl_label)+self.loss_ce(rl_output[1], rl_label)+self.loss_ce(rl_output[2], rl_label))/3
            # loss_ce = self.loss_ce(output[2], label_reindex_seen)
            # loss_cl = self.loss_ce(cl_output[2], label_reindex_seen)
            ...
            # loss_cl_value.append(loss_cl.data.item())
            # loss_rl_value.append(loss_rl.data.item())
            timer['model'] += self.split_time()

            # output = torch.cat([i.unsqueeze(1) for i in output], dim=1)
            # output = output.sum(1)
            value, spa_predict_label_fg = torch.max(output_spa[2].data, 1)
            value, spa_predict_label_mg = torch.max(output_spa[1].data, 1)
            value, spa_predict_label_cg = torch.max(output_spa[0].data, 1)
            value, tem_predict_label_fg = torch.max(output_tem[2].data, 1)
            value, tem_predict_label_mg = torch.max(output_tem[1].data, 1)
            value, tem_predict_label_cg = torch.max(output_tem[0].data, 1)
            # value, cat_predict_label = torch.max(output_label.data, 1)
            spa_pred_list_fg.append(spa_predict_label_fg.data.cpu().numpy())
            spa_pred_list_mg.append(spa_predict_label_mg.data.cpu().numpy())
            spa_pred_list_cg.append(spa_predict_label_cg.data.cpu().numpy())
            tem_pred_list_fg.append(tem_predict_label_fg.data.cpu().numpy())
            tem_pred_list_mg.append(tem_predict_label_mg.data.cpu().numpy())
            tem_pred_list_cg.append(tem_predict_label_cg.data.cpu().numpy())
            # cat_pred_list.append(cat_predict_label.data.cpu().numpy())
        ...
        spa_pred_list_fg = np.concatenate(spa_pred_list_fg)
        spa_pred_list_mg = np.concatenate(spa_pred_list_mg)
        spa_pred_list_cg = np.concatenate(spa_pred_list_cg)
        tem_pred_list_fg = np.concatenate(tem_pred_list_fg)
        tem_pred_list_mg = np.concatenate(tem_pred_list_mg)
        tem_pred_list_cg = np.concatenate(tem_pred_list_cg)
        # cat_pred_list = np.concatenate(cat_pred_list)
        # print(label_list[:20], pred_list[:20])
        spa_acc_fg = np.mean((spa_pred_list_fg==label_list))
        spa_acc_mg = np.mean((spa_pred_list_mg==label_list))
        spa_acc_cg = np.mean((spa_pred_list_cg==label_list))
        tem_acc_fg = np.mean((tem_pred_list_fg==label_list))
        tem_acc_mg = np.mean((tem_pred_list_mg==label_list))
        tem_acc_cg = np.mean((tem_pred_list_cg==label_list))
        # cat_acc = np.mean((cat_pred_list==label_list))
        pred_sum = ((spa_pred_list_fg==label_list)*1 + (spa_pred_list_mg==label_list)*1 + (spa_pred_list_cg==label_list)*1 + (tem_pred_list_fg==label_list)*1 + (tem_pred_list_mg==label_list)*1 + (tem_pred_list_cg==label_list)*1)
        pred_sum[pred_sum > 0] = 1
        acc_or = np.mean(pred_sum)
        self.print_log(
            '\tMean training loss: {:.4f}.'.format(np.mean(loss_value)))
        self.print_log(
            '\tMean training spatial fg acc: {:.2f}%.  Mean training spatial mg acc: {:.2f}%.  Mean training spatial cg acc: {:.2f}%.'.format(spa_acc_fg*100, spa_acc_mg*100, spa_acc_cg*100))
        self.print_log(
            '\tMean training temporal fg acc: {:.2f}%.  Mean training temporal mg acc: {:.2f}%.  Mean training temporal cg acc: {:.2f}%.'.format(tem_acc_fg*100, tem_acc_mg*100, tem_acc_cg*100))
        # self.print_log(
            # '\tMean training cat acc {:.2f}%.  '.format(cat_acc*100))
        self.print_log(
            '\tMean training or acc {:.2f}%.  '.format(acc_or*100))
        # self.print_log(
        #     '\tMean ce loss: {:.4f}.  Mean cl loss: {:.4f}.  Mean rl loss: {:.4f}.'.format(np.mean(loss_ce_value), np.mean(loss_cl_value), np.mean(loss_rl_value)))
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        label_semantics_unseen = self.label_semantics[unseen_classes].cuda(self.output_device)
        spatial_semantics_seen = self.spatial_semantics[seen_classes].cuda(self.output_device)
        spatial_semantics_unseen = self.spatial_semantics[unseen_classes].cuda(self.output_device)
        temporal_semantics_seen = self.temporal_semantics[seen_classes].cuda(self.output_device)
        temporal_semantics_unseen = self.temporal_semantics[unseen_classes].cuda(self.output_device)
        # zsl
        ...
        # gzsl
        gzsl_spatial_round_fg_unseen = self.spatial_round_fg[:120].cuda(self.output_device)
        gzsl_spatial_round_mg_unseen = self.spatial_round_mg[:120].cuda(self.output_device)
        gzsl_spatial_round_cg_unseen = self.spatial_round_cg[:120].cuda(self.output_device)
        gzsl_spatial_round = [gzsl_spatial_round_cg_unseen, gzsl_spatial_round_mg_unseen, gzsl_spatial_round_fg_unseen]
        gzsl_temporal_round_cg_unseen = self.temporal_round_cg[:120].cuda(self.output_device)
        gzsl_temporal_round_mg_unseen = self.temporal_round_mg[:120].cuda(self.output_device)
        gzsl_temporal_round_fg_unseen = self.temporal_round_fg[:120].cuda(self.output_device)
        ...
        for ln in loader_name:
            ...
            for batch_idx, (data, label, index) in enumerate(process):
                with torch.no_grad():
                    ...
                    if ln == "test_zsl":
                        ...
                        # spatial_semantics_unseen = self.spatial_semantics[unseen_classes].cuda(self.output_device)
                        # temporal_semantics_unseen = self.temporal_semantics[unseen_classes].cuda(self.output_device)
                        output_spa, output_tem, spa_feat, tem_feat, cube_feat, ta_feat = self.model(data, spatial_round, temporal_round, label_semantics_unseen)
                        # reading writing logits compare
                        # rw_logits_list.append(F.softmax(output[2], dim=1).data.cpu().numpy())
                        # sp_norm_feature_list.append(sp_norm_feature[2].data.cpu().numpy())
                        value, spa_predict_label_fg = torch.max(output_spa[2].data, 1)
                        value, spa_predict_label_mg = torch.max(output_spa[1].data, 1)
                        value, spa_predict_label_cg = torch.max(output_spa[0].data, 1)
                        value, tem_predict_label_fg = torch.max(output_tem[2].data, 1)
                        ...
                        # cat_pred_list.append(cat_predict_label.data.cpu().numpy())
                        for i in range(3):
                            spa_feat_list[i].append(spa_feat[i].data.cpu().numpy())
                            tem_feat_list[i].append(tem_feat[i].data.cpu().numpy())
                            ta_feat_list[i].append(ta_feat[i].data.cpu().numpy())
                        cube_feat_list.append(cube_feat.data.cpu().numpy())
                        label_list.append(label_reindex_unseen.data.cpu().numpy())
                    if ln == "test_gzsl":
                        label_semantics_unseen = self.label_semantics[:120,:].cuda(self.output_device)
                        label_reindex_unseen = torch.tensor([test_gzsl_label_map_dict[l.item()] for l in label]).cuda(self.output_device)
                        output_spa, output_tem, spa_feat, tem_feat, cube_feat, ta_feat = self.model(data, gzsl_spatial_round, gzsl_temporal_round, label_semantics_unseen)
                        # softmax
            ...
            if ln == 'test_zsl':
                ...
                ta_feat_array = []
                for i in range(3):
                    # print(len(spa_feat_list[i]))
                    # print(spa_feat_list[i][0].shape)
                    spa_feat_array.append(np.concatenate(spa_feat_list[i], axis=0))
                    tem_feat_array.append(np.concatenate(tem_feat_list[i], axis=0))
                    ta_feat_array.append(np.concatenate(ta_feat_list[i], axis=0))
                cube_feat_list = np.concatenate(cube_feat_list, axis=0)
                pred_sum_all = ((spa_pred_list_fg==label_list)*1 + (spa_pred_list_mg==label_list)*1 + (spa_pred_list_cg==label_list)*1 + (tem_pred_list_fg==label_list)*1 + (tem_pred_list_mg==label_list)*1 + (tem_pred_list_cg==label_list)*1)
                # pred_sum_all = ((tem_pred_list_fg==label_list)*1 + (tem_pred_list_mg==label_list)*1 + (tem_pred_list_cg==label_list)*1)
                # pred_sum = ((spa_pred_list_fg==label_list)*1 + (spa_pred_list_mg==label_list)*1 + (spa_pred_list_cg==label_list)*1)
                pred_sum_last = ((spa_pred_list_fg==label_list)*1+(tem_pred_list_fg==label_list)*1)
                # test = []
                # for pre_l, gt_l in zip(spa_pred_list_fg, label_list):
                #     if gt_l == 9:
                #         test.append(pre_l)
                # print(test)
                # pred_sum_last_and = ((spa_pred_list_fg==label_list)*1+(tem_pred_list_fg==label_list)*1)
                pred_sum_all[pred_sum_all > 0] = 1
                pred_sum_last[pred_sum_last > 0] = 1
                # pred_sum_last_and[pred_sum_last_and < 2] = 0
                # pred_sum_last_and[pred_sum_last_and == 2] = 1
                acc_or_all = np.mean(pred_sum_all)
                acc_or_last = np.mean(pred_sum_last)
                # acc_and_last = np.mean(pred_sum_last_and)
                # acc: each class

                label_onehot = np.eye(len(unseen_classes))[label_list]
                acc_each_class_all = np.dot(pred_sum_all.T, label_onehot)  # 1 5
                acc_each_class_all = acc_each_class_all / np.sum(label_onehot, axis=0)
                acc_each_class_last = np.dot(pred_sum_last.T, label_onehot)  # 1 5
                acc_each_class_last = acc_each_class_last / np.sum(label_onehot, axis=0)

                # acc_each_class_last_and = np.dot(pred_sum_last_and.T, label_onehot)  # 1 5
                # acc_each_class_last_and = acc_each_class_last_and / np.sum(label_onehot, axis=0)
                # rw_logits_list = np.concatenate(rw_logits_list)
                # sp_norm_feature_list = np.concatenate(sp_norm_feature_list)
                # np.savez('/root/autodl-tmp/FGE/visualization/unseen_spatial_fg_norm_feature_cl_rl.npz', feature=sp_norm_feature_list, label=label_list)
                # acc = np.mean(pred_list==label_list)
                print('*'*100)
                self.print_log('\tTop{} spatial fg: {:.2f}%'.format(1, spa_acc_fg*100))
                self.print_log('\tTop{} spatial mg: {:.2f}%'.format(1, spa_acc_mg*100))
                self.print_log('\tTop{} spatial cg: {:.2f}%'.format(1, spa_acc_cg*100))
                self.print_log('\tTop{} temporal fg: {:.2f}%'.format(1, tem_acc_fg*100))
                self.print_log('\tTop{} temporal mg: {:.2f}%'.format(1, tem_acc_mg*100))
                self.print_log('\tTop{} temporal cg: {:.2f}%'.format(1, tem_acc_cg*100))
                self.print_log('\tTop{} or all: {:.2f}%'.format(1, acc_or_all*100))
                print('Mean test unseen each class acc (all): ', acc_each_class_all)
                self.print_log('\tTop{} or last: {:.2f}%'.format(1, acc_or_last*100))
                print('Mean test unseen each class acc (last or): ', acc_each_class_last)
            ...
            if ln == 'test_gzsl':
                ...
                calibration_factor_spa_list = [i/100000 for i in range(10, 31, 1)]
                calibration_factor_tem_list = [i/100000 for i in range(10, 31, 1)]
                result_spa = []
                result_tem = []
                result = []
                # for cf_spa, cf_tem in zip(calibration_factor_spa_list, calibration_factor_tem_list):
                ...
                # for cf_spa, accuracy_spa_unseen, accuracy_spa_seen in result_spa:
                #     harmonic_mean_spa_acc = 2*accuracy_spa_seen*accuracy_spa_unseen/(accuracy_spa_seen+accuracy_spa_unseen)
                #     self.print_log('\tCalibration Spatial Factor: {:.8f}'.format(cf_spa))
                #     self.print_log('\tSeen Spa Acc: {:.2f}%'.format(accuracy_spa_seen*100))
                #     self.print_log('\tUnseen Spa Acc: {:.2f}%'.format(accuracy_spa_unseen*100))
                #     self.print_log('\tHarmonic Mean Spa Acc: {:.2f}%'.format(harmonic_mean_spa_acc*100))
                ...
                # print('*'*100)
                # for cf_tem, accuracy_tem_unseen, accuracy_tem_seen in result_tem:
                #     harmonic_mean_tem_acc = 2*accuracy_tem_seen*accuracy_tem_unseen/(accuracy_tem_seen+accuracy_tem_unseen)
                #     self.print_log('\tCalibration Temporal Factor: {:.8f}'.format(cf_tem))
                #     self.print_log('\tSeen Tem Acc: {:.2f}%'.format(accuracy_tem_seen*100))
                #     self.print_log('\tUnseen Tem Acc: {:.2f}%'.format(accuracy_tem_unseen*100))
                #     self.print_log('\tHarmonic Mean Tem Acc: {:.2f}%'.format(harmonic_mean_tem_acc*100))
                print('*'*100)
        ...
        # return spa_feat_array, tem_feat_array, label_list, cube_feat_list, ta_feat_array
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                # zsl
                # spa_feat, tem_feat, label, cube_feat, ta_feat = self.eval(epoch, save_score=self.arg.save_score, loader_name=['test_zsl'])
                ...
                # np.savez('/root/autodl-tmp/FGE/feature_folder/unseen_xsub_55_5_noforgetremember/cube_feature.npz', cube_feat=cube_feat, label=label)
                # np.savez('/root/autodl-tmp/FGE/feature_folder/unseen_xsub_55_5_noforgetremember/our_epoch{}.npz'.format(epoch+1), spa_feat=spa_feat, tem_feat=tem_feat, label=label)
                # for i in range(3):
                #     np.savez('/root/autodl-tmp/FGE/feature_folder/unseen_xsub_55_5_noforgetremember/ta_epoch{}_round{}.npz'.format(epoch+1, i+1), ta_feat=ta_feat[i], label=label)
                # break
                # gzsl
                # self.eval(epoch, save_score=self.arg.save_score, loader_name=['test_gzsl'])
